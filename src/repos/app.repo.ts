...
async getVideoConsumptionMs(fromMs: number, toMs: number, category?: string): Promise<number> {
  ...
  async getCurrentApp(): Promise<string | null> {
    const row = this.db.prepare(this._sql.getCurrentApp).get() as { app: string } | undefined
    return row?.app?? null
  }

  async getSummary(fromMs: number, toMs: number): Promise<AppSummaryRow[]> {
    return this.db.prepare(this._sql.getSummary).all([fromMs, toMs]) as AppSummaryRow[]
  }

  async getRecentActivity(limit: number): Promise<AppActivity[]> {
    return this.db.prepare(this._sql.getRecentActivity).all([limit]) as AppActivity[]
  }
