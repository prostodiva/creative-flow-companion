...
function formatNameStatus(stdout: string, limit: number): string {
  ...
  // `git diff --name-status` lines look like: "M\tpath/file.ts"
  return lines
...
export async function getTodoComments(cwd = process.cwd()): Promise<string[]> {
  try {
    const { stdout } = await execFileAsync('git', ['grep', '-n', '-E', 'TODO|FIXME'], {
      cwd,
      timeout: 2000,
    });
    ...
    return String(stdout ?? '')
      ...
      .slice(0, 3)
      .map((l) => {
        // format: file:line:content (or file:content depending on grep config)
        const parts = l.split(':');
    ...
    // `git grep` exits non-zero when no matches, and throws when not a repo.
    return [];
  }
}
